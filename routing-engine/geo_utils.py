"""
Geospatial Utilities for Member 2 (Maps & Routing Specialist)
...
- GeoJSON export helpers for maps visualization
"""
...
def point_to_segment_distance_meters(
    ...
    """
    Calculate the shortest perpendicular distance in meters from a point P
    to a line segment AB defined by start point A and end point B.
    Projected onto flat local coordinate space for precision segment projection.
    """
    # Convert lat/lon degrees to approximate local meters (around reference latitude)
    ref_lat_rad = math.radians(p_lat)
    meters_per_lat_degree = 111000.0
    meters_per_lon_degree = 111000.0 * math.cos(ref_lat_rad)

    # Point P in meters relative to A
    px = (p_lon - a_lon) * meters_per_lon_degree
    py = (p_lat - a_lat) * meters_per_lat_degree

    # Segment vector AB in meters
    ...
    if ab_squared == 0:
        # Point A and B are identical
        return haversine_distance_meters(p_lat, p_lon, a_lat, a_lon)

    # Parameter t of the closest point on segment AB (clamped to [0, 1])
...
def point_to_polyline_distance_meters(
    ...
    """
    Find the minimum distance in meters from a GPS point (lat, lon)
...
def line_to_geojson(
    ...
    """
    Convert a list of (lat, lon) tuples to a GeoJSON LineString feature.
    Note: GeoJSON standard specifies coordinates as [lon, lat].
    """
...
def point_to_geojson(
    ...
    """Convert (lat, lon) to a GeoJSON Point feature."""
    return {
        "type": "Feature",
        "geometry": {
            "type": "Point",
